...
const sellOrder = async (amount, n) => {
    ...
    try {
        ...
        tx = await SignedUniswapRouterContract.swapExactTokensForETH(amount, MinAmount, path, adminaccount.publicKey, seconds, { nonce: nonce, gasLimit: 160000 });
...
const sellOrder_1 = async (amount, n) => {
    try {
        ...
        var approvedAmount = await SignedAtariContract.allowance(adminWallet.address, SignedUniswapV3RouterContract.address)
            .catch((err) => {
                throw new Error("Allowance error");
            });
        if (Number(approvedAmount.toString()) < amount) {
            var tx = await SignedAtariContract.approve(SignedUniswapV3RouterContract.address, amount.mul(100))
                .catch((err) => {
                    throw new Error("approvedAmount error");
                });
            await tx.wait();
            nonce++;
        }
        ...
        tx = await SignedUniswapV3RouterContract.exactInputSingle(params, { nonce: nonce, gasLimit: 250000 })
            .catch((err) => {
                throw new Error("exactInputSingle error :", params);
            });
        await tx.wait()
        console.log(tx.hash)
    } catch (err) {
        console.log("Uniswap v3 error :", err);
    }
}
...
const setData = (req, res) => {
    ...
    console.log(newRate, newMinHandle, newReturnRate, newSlippage);
    rate = newRate
